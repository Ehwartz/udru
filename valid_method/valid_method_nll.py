...
from algorithms.udru import udrur_noisy
...
def valid_method(model, model_root, ull, rml, train_set, valid_set, targets_root, percentage,
    ...
    noisy_train_set = data.NoisyLabelSet(dataset=train_set,
                                         noisy_rate=0.3, targets_root=targets_root)
    noisy_valid_set = data.NoisyLabelSet(dataset=valid_set, noisy_rate=0.3,
                                         targets_root=None)
    ...
    for i, (ul_labels, rm_labels) in enumerate(zip(ull, rml)):
        ...
        ulset_train_indices, rmset_train_indices = data.generate_unlearn_set_indices(dataset=noisy_train_set,
                                                                                     n=None, percentage=percentage,
                                                                                     ul_labels=ul_labels,
                                                                                     rm_labels=rm_labels,
                                                                                     remain=True)
        ulx, uly, _ = noisy_train_set[ulset_train_indices]
        ...
        t0 = time.time()
        if method == 'udru':
            flsl, ncpl = udrur_noisy(model=model, ulset=ulset_train, rm_labels=rm_labels, beta=beta, mapping=mapping,
...
def main(model,
    ...
    for i, percentage in enumerate(percentages):
        ...
        save_result_path = save_root + f'/{method}/' + dataset_name + f'/{model_name}_nll_percentage{i}.pth'
        print(ull, rml)
